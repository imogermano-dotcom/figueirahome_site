...
import { Bath, BedDouble, MapPin, Ruler, Zap } from "lucide-react";
...
function mapProperty(row: ImovelDetailRow): Property | null {
  ...
  return {
    ...
    featured: false, published: true, agent_id: null, created_at: "", updated_at: "",
...
export function PropertyDetailBrowserFallback({ slug, reference }: { slug: string; reference?: string }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const fields = "imovel_ref,titulo,natureza,disponibilidade,estado,quartos,casas_banho,area_util,area_bruta,area_terreno,venda_preco,arrendamento_preco,concelho,freguesia,zona,descricao,certificacao_energetica,foto_principal,fotos,plantas,video_url,visita_virtual_url";
  ...
  return <><main className="pt-28"><section className="container"><PropertyGallery images={property.images || []} title={property.title} /><div className="mt-8 grid gap-10 lg:grid-cols-[1fr_380px]"><article><p className="text-sm font-extrabold uppercase tracking-[1.5px] text-[var(--gold)]">{property.type}</p><h1 className="section-title mt-2">{property.title}</h1><p className="mt-3 text-sm font-bold text-[var(--muted)]">Referência: <span className="text-[var(--text)]">{property.id}</span></p><p className="mt-3 flex items-center gap-2 text-[var(--muted)]"><MapPin size={18} />{property.location}</p><div className="display-font mt-6 text-3xl font-extrabold text-[var(--blue)]">{formatCurrency(property.price)}</div><div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Info icon={<BedDouble size={20} />} label="Quartos" value={String(property.bedrooms ?? "-")} /><Info icon={<Bath size={20} />} label="WC" value={String(property.bathrooms ?? "-")} /><Info icon={<Ruler size={20} />} label="Área" value={formatArea(property.area_sqm)} />{property.energy_certificate && <Info icon={<Zap size={20} />} label="Certificado energético" value={property.energy_certificate} />}</div><h2 className="mt-10 text-2xl font-extrabold">Descrição</h2><p className="mt-4 whitespace-pre-line leading-8 text-[var(--muted)]">{property.description}</p>{property.floor_plans?.length ? <section className="mt-10 border-l-4 border-[var(--gold)] pl-5"><p className="text-sm font-extrabold uppercase tracking-[1.5px] text-[var(--gold)]">Documentação do imóvel</p><h2 className="mt-2 text-2xl font-extrabold">Plantas do imóvel</h2><div className="mt-5"><PropertyGallery images={property.floor_plans} title={property.title} contentLabel="planta" /></div></section> : null}{property.video_url ? <section className="mt-10"><h2 className="text-2xl font-extrabold">Vídeo do imóvel</h2><div className="mt-4 overflow-hidden rounded-md border border-[var(--border)] bg-[var(--navy)]"><PropertyVideo url={property.video_url} title={property.title} /></div></section> : null}{property.visita_virtual_url ? <section className="mt-10"><h2 className="text-2xl font-extrabold">Visita virtual</h2><div className="mt-4 overflow-hidden rounded-md border border-[var(--border)] bg-[var(--navy)]"><PropertyVideo url={property.visita_virtual_url} title={`Visita virtual: ${property.title}`} /></div></section> : null}<h2 className="mt-10 text-2xl font-extrabold">Localização aproximada</h2><iframe title={`Mapa aproximado de ${property.location}`} src={mapEmbedUrl} className="mt-4 block h-[320px] w-full rounded-md border border-[var(--border)] sm:h-[380px]" loading="lazy" /></article><aside className="h-fit rounded-md border border-[var(--border)] p-6"><h2 className="mb-4 text-xl font-extrabold">Contactar sobre este imóvel</h2><ContactForm source="property_detail" propertyId={property.id} /></aside></div></section></main><VideoFooter /></>;
}
