...
Handoff operacional. Reescrito em 2026-09-02 — secção de estado consolidada, histórico de fixes já resolvidos removido. Manter este ficheiro abaixo de 200 linhas; substituir informação ultrapassada em vez de acumular.
...
- Produção: `https://figueirahome.pt` e `https://www.figueirahome.pt` (Worker Custom Domains). Preview/backup: `https://figueira-home.miguel-germano.workers.dev` (`workers_dev: true` em `wrangler.jsonc`). HEAD: `2f1a854` (branch `teste/alteracao-cliente`). Último deploy real de código: `06e4b4c5` (2026-09-02).
