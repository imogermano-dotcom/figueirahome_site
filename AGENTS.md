...
3. **Plano de teste Segurança/Conversão/Marca-Autoridade/SEO/RGPD — todas as 5 secções percorridas** (ver `C:\Users\joaoa\.claude\plans\moonlit-puzzling-rocket.md`). Pendências reais: headers CSP/HSTS (Segurança), RLS bloqueada pelo portal externo (Segurança), favicon + JSON-LD por página (SEO), confirmar testemunhos da homepage com o cliente (Marca).
4. Decidir com o cliente quem implementa o opt-out "PARAR" no backend do agente WhatsApp (`Figueirahome-Agent-call`) — a política de privacidade do site promete-o mas não existe no código.
5. Conseguir um ícone standalone (só a marca casa+seta, sem o texto) do cliente/designer para gerar o favicon.
