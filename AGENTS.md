...
- Idioma do site (Google Translate widget, `src/components/google-translate.tsx`): testado ao vivo, funciona — "voltar a Português" limpa o cookie `googtrans` e recarrega a página, ~4-5s (novo fetch do script do Google + reinit). Único feedback visual durante a espera é `opacity: .65` no `<select>` — pode ler-se como travado. Se o cliente voltar a reportar, considerar loading state mais visível antes de mexer na lógica (mecanismo em si não está partido).
