...
3. Plano de teste Segurança/Conversão/Marca-Autoridade/SEO/RGPD em curso — ver `C:\Users\joaoa\.claude\plans\moonlit-puzzling-rocket.md`. Segurança parcialmente executada (headers, secrets, npm audit, RLS — encontrou o bloqueio acima); faltam rate-limit em `/api/leads`/`/api/chat`, e as secções Conversão/Marca-Autoridade/SEO/RGPD.
