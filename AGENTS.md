...
4. Plano de teste Segurança/Conversão/Marca-Autoridade/SEO/RGPD em curso — ver `C:\Users\joaoa\.claude\plans\moonlit-puzzling-rocket.md`. Segurança quase fechada (falta só headers CSP/HSTS/etc e a RLS bloqueada pelo portal externo); RGPD fechado; SEO quase fechado (falta favicon — precisa de asset dedicado — e JSON-LD por página de imóvel/blog); Conversão quase fechada (falta re-testar o funil do chat quando o `/api/chat` for corrigido); falta a secção Marca-Autoridade (inclui rever o `border-l-4` em `imoveis/[slug]` L77 sinalizado pelo hook de design).
