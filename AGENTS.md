...
- **2026-09-01: chat AI interno (`/api/chat`) substituído pelo widget externo do portal.** Cliente forneceu `public/widget.js` (script autocontido, sem dependências) que fala com `https://figueirahome-agentos.fly.dev/api/site/chat` (agentes geridos no `figueira-home-portal`) — override via `window.FIGUEIRAHOME_CHAT_API` antes do `<script>`, se precisar apontar a outro host (staging, etc). Montado em `src/app/layout.tsx` via `next/script` (`strategy="afterInteractive"`), a seguir ao `<SiteChrome>`. Removidos como código morto: `src/components/chat-widget.tsx`, `src/app/api/chat/route.ts` (a rota `streamText`/`tools` com `searchProperties`/`getPropertyBySlug`/`createLead` descrita nas secções antigas deste ficheiro já não existe), `src/components/ai-elements/message.tsx` (órfão, só usado pelo chat antigo) e o CSS morto `.chat-widget`/`.chat-toggle`/`.chat-panel` em `globals.css`. O pacote `ai`/`@ai-sdk/*` fica em `package.json` sem uso conhecido — não removido nesta sessão, confirmar antes de tirar.
...
3. ~~Investigar o erro genérico do `/api/chat`~~ — obsoleto, rota removida em 2026-09-01 (substituída pelo widget do portal, ver "Bugs conhecidos").
4. **Plano de teste Segurança/Conversão/Marca-Autoridade/SEO/RGPD — todas as 5 secções percorridas** (ver `C:\Users\joaoa\.claude\plans\moonlit-puzzling-rocket.md`). Pendências reais: headers CSP/HSTS (Segurança), RLS bloqueada pelo portal externo (Segurança), favicon + JSON-LD por página (SEO), re-testar funil do chat (Conversão), confirmar testemunhos da homepage com o cliente (Marca).
